...
class BertGCN(nn.Module):
    ...
    def predict(self, src, label, mask, pos=None, vm=None):
        ...
        # 获取邻接矩阵的权重
        flat_src = src.reshape(-1)
        edge_index = torch.combinations(flat_src, r=2)
        edge_weights = torch.tensor([self.edge_matrix[row, col].item() for row, col in edge_index]).to(
            src.device)
...
def main():
    ...
    for epoch in tqdm(range(1, args.epochs_num + 1)):
        model.train()
        ...
        #other_input_ids = torch.LongTensor([example[4] for example in trainset])
        np_array = np.array([example[4] for example in trainset])
        # 然后将 NumPy 数组转换为张量
        other_input_ids = torch.LongTensor(np_array)
        other_mask_ids = torch.LongTensor([example[5] for example in trainset])
        other_pos_ids = torch.LongTensor([example[6] for example in trainset])
        other_vms = torch.LongTensor([example[7] for example in trainset])
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, other_input_ids_batch,
            ...
            #vms_batch = torch.LongTensor(vms_batch)
            vms_batch_np = np.array(vms_batch)  # 将列表转换为 NumPy 数组

            # 然后，将这个 NumPy 数组转换为一个 PyTorch 张量
            vms_batch = torch.LongTensor(vms_batch_np)
    ...
    if global_step % args.valid_steps == 0:
        precision, recall, f1 = evaluate(args, False)
        logger.info("evaluation precision: {}, recall: {}, F1: {}", precision, recall, f1)
        if f1 > best_f1:
            best_f1 = f1
